    ...
    >
      ...
      <section
        ...
        className={`py-20 px-4 sm:px-8 lg:px-16 ${
          isCuteMode ? "bg-gradient-to-r from-pink-200 to-purple-200" : "bg-gradient-to-r from-cyan-400 to-pink-400"
        }`}
      >
        <div className="max-w-7xl mx-auto">
          <h2
            className={`text-5xl sm:text-7xl font-black text-center mb-6 transform -rotate-1 ${
          ...
          <p
            className={`text-center text-xl mb-12 max-w-2xl mx-auto font-bold transform rotate-1 ${
            ...
            {[
              ...
              <div
                ...
                className={`p-8 transform hover:scale-105 transition-all ${
                  isCuteMode ? "bg-white rounded-3xl shadow-lg hover:shadow-xl" : "bg-white shadow-[6px_6px_0_0_#000] hover:shadow-[10px_10px_0_0_#000]"
                }`}
              ...
              >
                ...
                <p className={`leading-relaxed ${isCuteMode ? "text-gray-600 font-medium" : "text-gray-700 font-bold"}`}>
                  {feature.description}
                </p>
              </div>
            ))}
      ...
      <section
        id="featured"
        className={`py-20 px-4 sm:px-8 lg:px-16 ${
          isCuteMode ? "bg-gradient-to-b from-pink-100 to-purple-100" : "bg-gradient-to-b from-yellow-200 to-orange-300"
        }`}
      >
        <div className="max-w-7xl mx-auto">
          <h2
            className={`text-5xl sm:text-7xl font-black text-center mb-6 transform rotate-2 ${
              isCuteMode ? "text-purple-700" : "text-orange-800 drop-shadow-[4px_4px_0_#000]"
            }`}
          >
            {isCuteMode ? "Adorable Pets 💎" : "THE ICONS 💎"}
          </h2>
          <p className={`text-center text-xl mb-12 max-w-2xl mx-auto font-bold transform -rotate-1 ${isCuteMode ? "text-purple-600" : "text-orange-900"}`}>
            ...
            {[
              ...
              <div
                ...
                className={`bg-white overflow-hidden transform hover:scale-105 transition-all ${
                  isCuteMode ? "rounded-3xl shadow-lg hover:shadow-xl" : "shadow-[6px_6px_0_0_#000] hover:shadow-[10px_10px_0_0_#000]"
                }`}
                style={{ transform: `rotate(${index % 2 === 0 ? 2 : -2}deg)` }}
              >
                <div
                  className={`h-56 flex items-center justify-center ${
                    isCuteMode ? "bg-gradient-to-br from-pink-200 to-purple-200" : "bg-gradient-to-br from-pink-300 to-purple-400"
                  }`}
                >
                ...
                <div className="p-6">
                  <div className="flex justify-between items-start mb-3">
                    <h3 className={`text-2xl font-black ${isCuteMode ? "text-purple-700" : "text-purple-800"}`}>
                      {pet.name}
                    </h3>
                    <div className={`px-3 py-1 font-black text-sm transform -rotate-2 ${isCuteMode ? "bg-pink-300 text-purple-700 rounded-full" : "bg-yellow-400"}`}>
                      {pet.rating}
                    </div>
                  </div>
                  <p className={`mb-1 ${isCuteMode ? "text-gray-600 font-medium" : "text-gray-600 font-bold"}`}>{pet.type}</p>
                  <p className={`text-sm mb-4 ${isCuteMode ? "text-purple-500 font-semibold" : "text-pink-600 font-black uppercase"}`}>{pet.vibe}</p>
                  <button className={`w-full py-3 font-semibold uppercase tracking-wider transition-colors ${isCuteMode ? "bg-purple-400 text-white rounded-full hover:bg-purple-500 shadow-lg" : "bg-purple-600 text-white font-black hover:bg-purple-800 shadow-[4px_4px_0_0_#000] hover:shadow-[6px_6px_0_0_#000]"}`}>
      ...
      <section className={`py-20 px-4 sm:px-8 lg:px-16 ${isCuteMode ? "bg-gradient-to-r from-purple-200 to-pink-200" : "bg-gradient-to-r from-green-400 to-cyan-400"}`}>
        <div className="max-w-7xl mx-auto">
          <h2 className={`text-5xl sm:text-7xl font-black text-center mb-6 transform -rotate-2 ${isCuteMode ? "text-purple-700" : "text-white drop-shadow-[4px_4px_0_#000]"}`}>
            {isCuteMode ? "Happy Pet Lovers 💕" : "THE TEA ☕"}
          </h2>
          <p className={`text-center text-xl mb-12 max-w-2xl mx-auto font-bold transform rotate-1 ${isCuteMode ? "text-purple-600" : "text-white"}`}>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {[
              ...
              <div key={index} className={`p-8 transform hover:scale-105 transition-all ${isCuteMode ? "bg-white rounded-3xl shadow-lg hover:shadow-xl" : "bg-white shadow-[6px_6px_0_0_#000]"}`} style={{ transform: `rotate(${index % 2 === 0 ? -2 : 2}deg)` }}>
                <div className="flex mb-4 text-3xl">{[...Array(testimonial.rating)].map((_, i) => (<span key={i}>💖</span>))}</div>
                <p className={`leading-relaxed mb-6 italic ${isCuteMode ? "text-gray-700 font-medium" : "text-gray-800 font-bold"}`}>
                  "{testimonial.text}"
                </p>
                <div className="flex items-center gap-4">
                  <div className={`w-16 h-16 rounded-full flex items-center justify-center text-3xl ${isCuteMode ? "bg-gradient-to-br from-pink-300 to-purple-300" : "bg-gradient-to-br from-pink-400 to-purple-500"}`}>
                    {isCuteMode ? "�" : "�"}
                  </div>
                  <div>
                    <p className={`font-black ${isCuteMode ? "text-purple-700" : "text-purple-800"}`}>{testimonial.name}</p>
                    <p className={`text-xs uppercase tracking-widest font-semibold ${isCuteMode ? "text-purple-500" : "text-pink-600 font-bold"}`}>{testimonial.title}</p>
                  </div>
                </div>
              </div>
            ))}
      ...
      {/* CTA Section */}
      <section className={`py-20 px-4 sm:px-8 lg:px-16 ${isCuteMode ? "bg-gradient-to-r from-pink-300 to-purple-300" : "bg-gradient-to-r from-red-500 to-orange-500"}`}>
        <div className="max-w-5xl mx-auto text-center">
          <div className="mb-8 animate-bounce">
            <span className="text-8xl">{isCuteMode ? "🌸" : "🎪"}</span>
          </div>
          <h2 className={`text-5xl sm:text-7xl font-black text-white mb-8 transform -rotate-1 ${isCuteMode ? "" : "drop-shadow-[4px_4px_0_#000]"}`}>
            {isCuteMode ? "Join Our Family 🎉" : "JOIN THE MADNESS 🎉"}
          </h2>
          <p className={`text-2xl mb-12 max-w-2xl mx-auto font-bold transform rotate-1 ${isCuteMode ? "text-purple-700" : "text-white"}`}>
          ...
          <Link href='/main' className={`px-12 py-6 font-semibold text-xl uppercase tracking-widest transform hover:scale-110 transition-all ${isCuteMode ? "bg-white text-purple-600 rounded-full shadow-lg" : "bg-white text-red-600 font-black shadow-[8px_8px_0_0_#000] hover:shadow-[12px_12px_0_0_#000]"}`}>
